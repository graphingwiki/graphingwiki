...
class DeletePage(ActionBase):
    ...
    def do_action(self):
        #raippa delete
        if 'raippa_delete' in self.form.keys():
            types = self.form.get('raippa_delete', list())
            ...
            if 'question' in types:
                ...
                success, msg = question.delete(comment)
                return success, [msg]
            elif 'task' in types:
                task = Task(self.request, self.pagename)
                success, msg = task.delete(comment)
                return success, [msg]
            else:
    ...
    def get_form_html(self, buttons_html):
        ...
        if type:
            ...
            return ''' 
<input type="hidden" name="raippa_delete" value="%(type)s">
