...
class MyClientHandler( SocketServer.BaseRequestHandler ):
    def handle( self ):
        #empty analysis folder, if it's not empty
        os.system( "rm /var/www/malwarezor/*" ) 
        #mount malware cd from defined .iso
        ...
        # except for knowing exactly what malware are in operation. In
        ...
        if( directories.count > 1 ):
            ...
            self.request.send( malwareSampleName )#send mw name to the victim over TCP/IP
        else:
            self.request.close()
