...
from graphingwiki.editing import save_attachfile # tst
from graphingwiki.editing import load_attachfile # tst
from graphingwiki.editing import delete_attachfile # tst
from graphingwiki.editing import list_attachments # tst
...
def save_meta(srcWiki, wiki, key, out, createpage=True,
            category_edit='', catlist=[], template=''):
    retval = []
    succ = xmlrpc_connect(srcWiki.SetMeta, wiki, key, out, 'repl',
                          createpage, category_edit, catlist, template)
    if isinstance(succ, dict):
        code, reason = xmlrpc_error(succ)
        retval.append('%s: save failed!' % (key))
        retval.append("Error: %s %s" % (code, reason))
    else:
        retval.extend(succ)
    return retval
...
# out = {'key': ['value1', 'value2']}
# print save_meta(srcWiki, wiki, 'page', out)
